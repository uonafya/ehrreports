...
public class PatientOccurenceCalculation extends AbstractPatientCalculation {
	...
	public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues,
		...
		EhrCalculationService ePTSCalculationService = Context.getRegisteredComponents(EhrCalculationService.class).get(0);
		CalculationResultMap resultMap = new CalculationResultMap();
		EhrReportConstants.OccurenceStates state = (EhrReportConstants.OccurenceStates) parameterValues.get("state");
		
		CalculationResultMap allEncounters = ePTSCalculationService.allEncounters(null, cohort, null, context.getNow(),
		    context);
		for (Integer pId : cohort) {
			...
			if (state.equals(EhrReportConstants.OccurenceStates.NEW) && encounterList.size() == 1) {
				isCandidate = true;
			} else if (state.equals(EhrReportConstants.OccurenceStates.REVISIT) && encounterList.size() > 1) {
				isCandidate = true;
			}
